...
3. 解析层：字段提取器 + 归一化引擎。
4. 数据层：PostgreSQL（结构化）+ 本地/对象存储（原始快照）。
5. 交付层：Excel 导出与运行摘要。
...
3. 数据处理：`pandas`
4. 数据库：`sqlalchemy` + `psycopg2`
5. 导出：`openpyxl`（Excel）
6. 日志：`structlog` 或标准 `logging`（JSON 格式）

### 4.3 选型理由
1. Hikvision/Dahua 页面可能存在前端渲染，`playwright` 能提高稳定性。
2. `pandas + openpyxl` 可快速输出固定模板 Excel。
3. PostgreSQL 适合结构化查询、增量比较、异常追踪。
