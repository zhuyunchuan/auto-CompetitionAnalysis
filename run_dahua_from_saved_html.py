#!/usr/bin/env python3
"""
Full Dahua collection using saved HTML files.
...
# Known Dahua WizSense 3 products (from previous data)
WIZSENSE_3_PRODUCTS = [
...
# Add more models to reach closer to 41
MORE_WIZSENSE_3 = [
    ("IPC-HFW3541E", "https://www.dahuasecurity.com/products/network-products/network-cameras/wizsense-3-series/ipc-hfw3541e"),
    ("IPC-HDBW3541E", "https://www.dahuasecurity.com/products/network-products/network-cameras/wizsense-3-series/ipc-hdbw3541e"),
    ("IPC-HFW3541T", "https://www.dahuasecurity.com/products/network-products/network-cameras/wizsense-3-series/ipc-hfw3541t"),
    ("IPC-HDBW3541T", "https://www.dahuasecurity.com/products/network-products/network-cameras/wizsense-3-series/ipc-hdbw3541t"),
    ("IPC-HFW4431E", "https://www.dahuasecurity.com/products/network-products/network-cameras/wizsense-3-series/ipc-hfw4431e"),
    ("IPC-HDBW4431E", "https://www.dahuasecurity.com/products/network-products/network-cameras/wizsense-3-series/ipc-hdbw4431e"),
]

ALL_PRODUCTS = WIZSENSE_3_PRODUCTS + MORE_WIZSENSE_3
...
def main():
    ...
    try:
        ...
        for model, url in ALL_PRODUCTS:
            if model in seen_models:
                continue
            seen_models.add(model)

            # Try to find saved HTML file (files are named with https___ prefix)
            html_filename = url.replace("https://", "https___").replace("http://", "http___").replace("/", "_")
            html_files = list(raw_html_dir.glob(f"*{html_filename}"))

            if html_files:
                html_path = html_files[0]
                print(f"   Found HTML for {model}: {html_path.name}")
                unique_products.append((model, url, str(html_path)))
            else:
                print(f"   ⚠ No HTML found for {model}")
